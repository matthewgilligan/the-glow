...
const ReviewsPage = () => {
  const data = useStaticQuery(graphql`
    query {
      allContentfulReview ( sort: { fields:publishedDate, order:DESC }, filter: { reviewCategory:{ name: { eq: "Classic" } }, genre: { name:{ eq: "Rock"} } } ) {
        edges {
          node {
            albumTitle
            slug
      ...
      <div className={stickyNavStyles.nav}>
        ...
        <div className={stickyNavStyles.navItems}>
          ...
          <div class={stickyNavStyles.dropdown}>
            <button class={stickyNavStyles.hipHopButton}>Rock ▾</button>
