...
const ReviewsPage = () => {
  const data = useStaticQuery(graphql`
    query {
      allContentfulReview ( sort: { fields:publishedDate, order:DESC }, filter: { reviewCategory:{ name: { eq: "Contemporary" } }, genre: { name:{ eq: "Pop"} } } ) {
        edges {
          node {
            albumTitle
            slug
      ...
      <div className={stickyNavStyles.nav}>
        ...
        <div className={stickyNavStyles.navItems}>
          ...
          <div class={stickyNavStyles.dropdown}>
            <button class={stickyNavStyles.popButton}>Pop ▾</button>
